...
async function storageGet(key){
  ...
  /* علامة صغيرة تقول أين هبط آخر حفظ ناجح فعليًا — تحسّم أي التباس بين IndexedDB وlocalStorage
     في حال فشل الحفظ في IndexedDB لمرة عابرة وسقط تلقائيًا إلى localStorage: بدونها كنا سنقرأ
     دائمًا من IndexedDB أولاً حتى لو كانت نسخته أقدم من التي وصلت فعلاً إلى localStorage، فتبدو
     الشواهد الأحدث وكأنها اختفت رغم أنها محفوظة فعلاً. */
  let backend = null;
  try { backend = localStorage.getItem(key + "__backend"); } catch(e){}
  if (backend === "ls") {
...
async function storageSet(key, value){
  ...
  try {
    ...
    /* بعد أول نجاح للحفظ في IndexedDB، نحذف النسخة القديمة من localStorage لتحرير مساحتها الصغيرة،
       ونثبّت العلامة على "idb" ليعرف storageGet أين المصدر الصحيح */
    try { localStorage.setItem(key + "__backend", "idb"); localStorage.removeItem(key); } catch(e){}
    return true;
  } catch(e){
    try {
      localStorage.setItem(key, value);
      try { localStorage.setItem(key + "__backend", "ls"); } catch(e2){}
      return true;
...
function ensureDataShape(){
  ...
  /* الفعاليات المضافة سابقًا (قبل إضافة خيار "مرة واحدة") كانت جميعها متكررة أسبوعيًا بلا استثناء —
      ...
      if (snap.exists) {
        ...
        if (remote && remote.payload) {
          try {
            ...
            /* يجب حفظ نتيجة الدمج محليًا فورًا — وإلا فهي تبقى في الذاكرة فقط، وإذا أُعيد تحميل
               الصفحة قبل أي تعديل محلي آخر (الذي كان سابقًا الوسيلة الوحيدة لاستدعاء الحفظ) تضيع
               هذه التحديثات القادمة من الجهاز الآخر تمامًا وكأنها لم تصل أبدًا — بالضبط ما يظهر
               للمستخدم كأن "المزامنة لا تعمل" رغم أنها استقبلت التحديث فعليًا لكنه لم يُحفظ */
            persist();
...
function viewDashboard(){
  ...
  const todaysWeekly = DATA.weekly.filter(w => w.recurring ? w.day === today.dayName : w.date === todayStr);
  ...
  return `
    <div class="evidence-panel">
      ...
      <div style="border-top:1px dashed var(--line); padding-top:12px; margin-bottom:12px;">
        <div style="font-size:11.5px; color:var(--muted); margin-bottom:8px;">إذا لا تحتاج تسجيل أسماء الطلاب، اكتفِ بعدد فقط:</div>
        <div style="display:flex; gap:8px; flex-wrap:wrap; align-items:center;">
          <button class="btn small ghost" data-action="markAllStudentsParticipated" data-id="${p.id}">✓ جميع الطلاب شاركوا (${p.targetCount})</button>
          <input id="ps-count-${p.id}" type="number" min="0" placeholder="أو عدد محدد" style="width:120px; border:1px solid var(--line); border-radius:8px; padding:8px 10px; font-family:'Cairo'; font-size:12.5px;">
          <button class="btn small" data-action="setPlanProgramCount" data-id="${p.id}">حفظ العدد</button>
        </div>
      </div>
...
/* تعديل برنامج موجود — يسمح بتغيير تاريخه مباشرة (مثلاً من الأحد إلى الاثنين) دون حاجة لحذفه
   وإعادة إضافته، مع الحفاظ التام على شواهده وطلابه المسجَّلين كما هم دون أي مساس بهم */
function inlinePlanProgramEditForm(p){
  return `
    <div class="panel">
      <div class="field full"><label>اسم البرنامج</label><input id="pl-edit-name" type="text" value="${esc(p.name)}"></div>
      <div class="field"><label>الفصل الدراسي</label>
        <select id="pl-edit-semester">
          <option value="1" ${p.semester===1?"selected":""}>الفصل الأول</option>
          <option value="2" ${p.semester===2?"selected":""}>الفصل الثاني</option>
        </select>
      </div>
      <div class="field"><label>رقم الأسبوع</label><input id="pl-edit-week" type="number" min="1" max="20" value="${p.week}"></div>
      <div class="field"><label>التاريخ الفعلي</label><input id="pl-edit-date" type="date" value="${esc(p.date)}"></div>
      <div class="field"><label>عدد الطلاب المستهدف</label><input id="pl-edit-target" type="number" min="0" value="${p.targetCount}"></div>
      <div class="field"><label>مكان الفعالية</label><input id="pl-edit-location" type="text" value="${esc(p.location||"")}"></div>
      <div class="field full"><label>وصف مختصر</label><textarea id="pl-edit-desc" rows="2">${esc(p.desc||"")}</textarea></div>
      <div class="actions">
        <button class="btn ghost" data-action="closePlanProgramEdit">إلغاء</button>
        <button class="btn" data-action="savePlanProgramEdit" data-id="${p.id}">حفظ التعديل</button>
      </div>
    </div>`;
}
...
function viewActivityPlan(){
  ...
  const catCards = cats.map(cat => {
    const rows = [...cat.programs].sort((a,b)=>a.date.localeCompare(b.date)).map(p => {
      ...
      const panelRows = [];
      if (EDIT_PLAN_PROGRAM_ID === p.id) panelRows.push(`<tr class="evidence-row"><td colspan="4">${inlinePlanProgramEditForm(p)}</td></tr>`);
      ...
      <tr class="${p.done?'done-row':''}">
        ...
        <td>
          <div class="tt">${esc(p.name)}${p.done?' <span style="color:var(--cyan); text-decoration:none;">· ✓ منتهي</span>':""}</div>
        ...
        <td style="width:280px;">
          <div style="display:flex; align-items:center; gap:6px; flex-wrap:wrap;">
            ...
            <button class="icon-btn" data-action="togglePlanProgramDone" data-id="${p.id}" title="${p.done?'إلغاء الإتمام':'إتمام'}">${ICONS.check}</button>
            <button class="icon-btn" data-action="editPlanProgram" data-id="${p.id}" title="تعديل البرنامج">${ICONS.pencil}</button>
...
function viewVision(){
  ...
  return `
    <div class="topbar">
      ...
      <button class="btn ghost" data-action="reportPrint">🖨️ طباعة</button>
    </div>
    ${moeFrame}
...
function weeklyFormPanel(){
  ...
  /* الفعالية الجديدة تكون لمرة واحدة افتراضيًا (بتاريخ محدد) ولا تتكرر أسبوعيًا إلا إذا فُعِّل الخيار صراحة */
  const isRecurring = editing ? !!editing.recurring : false;
  const defaultDay = (editing && editing.day) || NEW_WEEKLY_DAY || DAYS[0];
  const defaultDate = (editing && editing.date) || "";
  return `
    <div class="panel">
      <div class="field full" style="display:flex; align-items:center; gap:8px;">
        <input id="w-recurring" type="checkbox" ${isRecurring?"checked":""} onchange="onWeeklyRecurringChange()" style="width:auto;">
        <label style="margin:0;">تكرار أسبوعيًا (بدل مرة واحدة بتاريخ محدد)</label>
      </div>
      <div class="field ${isRecurring?"":"hidden"}" id="w-day-wrap"><label>اليوم</label>
        <select id="w-day">${DAYS.map(d=>`<option value="${d}" ${defaultDay===d ? "selected":""}>${d}</option>`).join("")}</select>
      </div>
      <div class="field ${isRecurring?"hidden":""}" id="w-date-wrap"><label>التاريخ</label><input id="w-date" type="date" value="${esc(defaultDate)}"></div>
...
function inlineWeeklyEditForm(w){
  ...
  return `
    <div class="inline-edit">
      ...
      <div class="field ${isRecurring?"hidden":""}" id="w-date-wrap"><label>التاريخ</label><input id="w-date" type="date" value="${esc(w.date||"")}"></div>
...
function viewWeekly(){
  ...
  /* نطاق الأسبوع الحالي (الأحد إلى السبت) — الفعاليات المتكررة تظهر دائمًا حسب يومها، والفعاليات
     لمرة واحدة تظهر فقط إن وقع تاريخها ضمن هذا الأسبوع بالذات */
  const weekStart = new Date(_now); weekStart.setDate(_now.getDate() - _now.getDay());
  const weekDateByDay = {};
  for (let i=0;i<7;i++){
    const d2 = new Date(weekStart); d2.setDate(weekStart.getDate()+i);
    weekDateByDay[DAY_INDEX[d2.getDay()]] = isoDate(d2.getFullYear(), d2.getMonth(), d2.getDate());
  }

  const cols = DAYS.map(day => {
    const items = DATA.weekly.filter(w => w.recurring ? w.day === day : w.date === weekDateByDay[day]).sort((a,b)=>a.time.localeCompare(b.time));
    return `
      <div class="day-col">
...
/* المهام */
/* ============================================================ */
/* البحث عن مصفوفة/عنصر شاهد بحسب kind — تُستخدم لعرض الفيديو عند الطلب دون تضمين بياناته
   الكاملة (قد تصل لعشرات الميجابايت) داخل نص HTML المُعاد بناؤه في كل render() */
...
function evidenceThumb(e, kind){
  if (e.type === "video") {
    /* لا نضع بيانات الفيديو (dataUrl) هنا إطلاقًا — تضمين فيديوهات متعددة كاملة داخل نص HTML
...
function getDayItems(iso, weekdayName){
  /* الفعاليات المتكررة لا تظهر إلا بدءًا من بداية العام الدراسي — قبل ذلك لا توجد دراسة أصلًا.
     الفعاليات لمرة واحدة تظهر فقط في تاريخها المحدد بالضبط. */
  const weeklyEvents = DATA.weekly.filter(w =>
    w.recurring ? (w.day === weekdayName && iso >= SY_START) : (w.date === iso)
  ).sort((a,b)=>a.time.localeCompare(b.time));
  const tasksDue = DATA.tasks.filter(t => t.dueDate === iso);
  const programsDue = (DATA.activityPlan && DATA.activityPlan.categories || []).flatMap(cat =>
    (cat.programs||[]).filter(p => p.date === iso).map(p => ({...p, catName: cat.name})));
  const planEntry = DATA.weeklyPlan.find(p => p.startDate && p.endDate && iso >= p.startDate && iso <= p.endDate);
  return {weeklyEvents, tasksDue, programsDue, planEntry};
}
...
function buildTableRows(){
  ...
  for (let d=1; d<=daysInMonth; d++){
    ...
    const {weeklyEvents, tasksDue, programsDue, planEntry} = getDayItems(iso, weekdayName);
    const items = [];
    weeklyEvents.forEach(w => items.push(
      EDIT_WEEKLY_ID === w.id ? inlineWeeklyEditForm(w) :
      `<div style="display:flex; align-items:center; gap:6px;">🕐 <b>${esc(w.time)}</b> — ${esc(w.title)}${w.location?` <span style="color:var(--muted)">(${esc(w.location)})</span>`:""}<button class="icon-btn" data-action="editWeekly" data-id="${w.id}" style="margin-inline-start:auto;">${ICONS.pencil}</button><button class="trash-btn" data-action="removeWeekly" data-id="${w.id}">${ICONS.trash}</button></div>`
    ));
    tasksDue.forEach(t => items.push(`<div style="color:${t.done?'var(--muted)':'var(--coral)'}">${t.done?"✔":"◻"} ${esc(t.title)}</div>`));
    programsDue.forEach(p => items.push(`<div style="color:var(--navy)">🎯 ${esc(p.name)} <span style="color:var(--muted)">(${esc(p.catName)})</span></div>`));
...
function viewCalendar(){
  ...
  for (let d=1; d<=daysInMonth; d++){
    ...
    programsDue.forEach(p => labels.push(`<div class="cal-label" style="background:#EAF1FC; color:var(--navy);">🎯 ${esc(p.name)}</div>`));
  ...
  const detailBody = `
    ...
    ${sel.programsDue.length ? `
      <div style="font-weight:800;color:var(--navy);margin-bottom:8px;font-size:13px; margin-top:16px;">برامج خطة رائد النشاط</div>
      <div class="list">
        ${sel.programsDue.map(p=>{
          if (EDIT_PLAN_PROGRAM_ID === p.id) return `<div class="item" style="display:block;">${inlinePlanProgramEditForm(findPlanProgram(p.id))}</div>`;
          const evN=(p.evidence||[]).length;
          const panel = EVIDENCE_OPEN_ID === p.id ? `<div class="item" style="display:block;">${evidencePanel(p,"planprogram")}</div>` : "";
          return `
          <div class="item">
            <div class="flex1">
              <div class="title" style="${p.done?'text-decoration:line-through;opacity:.65;':''}">🎯 ${esc(p.name)}${p.done?' <span style="color:var(--cyan); text-decoration:none;">· ✓ منتهي</span>':""}</div>
              <div class="meta">${esc(p.catName)}${evN?` · 📎 ${evN} شاهد`:""}</div>
            </div>
            <button class="evidence-btn ${evN?'has':''}" data-action="toggleEvidence" data-id="${p.id}" title="الشواهد">📎${evN?` ${evN}`:""}</button>
            <button class="icon-btn" data-action="togglePlanProgramDone" data-id="${p.id}" title="${p.done?'إلغاء الإتمام':'إتمام'}">${ICONS.check}</button>
            <button class="icon-btn" data-action="editPlanProgram" data-id="${p.id}" title="تعديل البرنامج">${ICONS.pencil}</button>
            <button class="trash-btn" data-action="removePlanProgram" data-id="${p.id}" title="حذف البرنامج">${ICONS.trash}</button>
          </div>${panel}`;
        }).join("")}
      </div>` : ""}
  `;
...
function buildReportData(startDate, endDate){
  ...
  while (d <= end){
    ...
    DATA.weekly.forEach(w => {
      const matches = w.recurring ? (w.day===wd && iso >= SY_START) : (w.date === iso);
      if (matches) weeklyEvents.push({date: iso, ...w});
    });
...
function reportHtml(title, rangeLabel, data){
  ...
  /* نسبة الإنجاز العامة (تظهر في الغلاف) يجب أن تعكس العمل الفعلي كله في الفترة — لا المهام فقط،
     ...
     تنفيذ عدة فعاليات فعليًا بشواهد. نبقي أيضًا نسبة إنجاز المهام وحدها منفصلة لعرضها في سياقها الخاص. */
  const taskPct = data.tasksInRange.length ? Math.round(done.length/data.tasksInRange.length*100) : 0;
  const doneEvents = data.weeklyEvents.filter(w => { const lg = getLog(w.id, w.date); return lg && lg.done; }).length;
  const totalForPct = data.tasksInRange.length + data.weeklyEvents.length;
  const doneForPct = done.length + doneEvents;
  const pct = totalForPct ? Math.round(doneForPct/totalForPct*100) : 0;
  ...
  const events = `
    <div class="rpt-page">
      ...
      <div class="rpt-domain" style="background:var(--cyan); margin-top:30px;">
        ...
        <div class="pill">نسبة إنجاز المهام ${taskPct}%</div>
      </div>
...
function onWeeklyRecurringChange(){
  const cb = document.getElementById("w-recurring");
  const dayWrap = document.getElementById("w-day-wrap");
  const dateWrap = document.getElementById("w-date-wrap");
  if (!cb || !dayWrap || !dateWrap) return;
  dayWrap.classList.toggle("hidden", !cb.checked);
  dateWrap.classList.toggle("hidden", cb.checked);
}
    ...
    ${VIDEO_LIGHTBOX ? `
      <div class="video-lightbox">
        <div class="video-lightbox-inner" id="video-lightbox-inner">
          <button class="video-lightbox-close" data-action="closeVideoLightbox">${ICONS.x}</button>
        </div>
      </div>` : ""}
  `;
...
/* الفيديو الفعلي (dataUrl قد يصل لعشرات الميجابايت) لا يُمرَّر أبدًا داخل نص HTML المُعاد بناؤه —
   يُنشأ عنصر <video> عبر الـ DOM مباشرة وتُضبط بياناته كخاصية JS، وليس كنص داخل القالب، حتى لا
   يتكرر تحميل/فك ترميز كل الفيديوهات المعروضة سابقًا مع كل render() (كان هذا سبب تعليق/إغلاق
   الصفحة عند رفع فيديو جديد وشواهد فيديو أخرى مفتوحة أو محفوظة) */
function mountVideoLightbox(){
  if (!VIDEO_LIGHTBOX) return;
...
document.addEventListener("click", async (e) => {
  ...
  if (action === "addWeekly") {
    const recurring = document.getElementById("w-recurring").checked;
    const day = recurring ? document.getElementById("w-day").value : "";
    const date = recurring ? "" : document.getElementById("w-date").value;
    ...
    await mutate(d => { d.weekly.push({id:uid(), recurring, day, date, time, title, location}); });
  ...
  if (action === "saveWeekly") {
    ...
    await mutate(d => {
      ...
      if (w) { w.recurring=recurring; w.day=day; w.date=date; w.time=time; w.title=title; w.location=location; }
    });
  ...
  if (action === "editPlanProgram") {
    EDIT_PLAN_PROGRAM_ID = (EDIT_PLAN_PROGRAM_ID === btn.dataset.id ? null : btn.dataset.id);
    render();
    return;
  }
  if (action === "togglePlanProgramDone") {
    const id = btn.dataset.id;
    await mutate(() => { const p = findPlanProgram(id); if (p) p.done = !p.done; });
    return;
  }
  if (action === "closePlanProgramEdit") { EDIT_PLAN_PROGRAM_ID = null; render(); return; }
  if (action === "savePlanProgramEdit") {
    const id = btn.dataset.id;
    const name = document.getElementById("pl-edit-name").value.trim();
    const semester = parseInt(document.getElementById("pl-edit-semester").value, 10);
    const week = parseInt(document.getElementById("pl-edit-week").value, 10) || 1;
    const date = document.getElementById("pl-edit-date").value;
    const targetCount = parseInt(document.getElementById("pl-edit-target").value, 10) || 0;
    const location = document.getElementById("pl-edit-location").value.trim();
    const desc = document.getElementById("pl-edit-desc").value.trim();
    if (!name || !date) return;
    /* نُحدِّث حقول البرنامج فقط (الاسم، الفصل، الأسبوع، التاريخ الفعلي المُدخَل مباشرة، المستهدف،
       المكان، الوصف) — مع الإبقاء التام على id والطلاب المسجَّلين والشواهد المرفوعة كما هي تمامًا،
       فلا خطر إطلاقًا على أي بيانات أو شواهد محفوظة مسبقًا لهذا البرنامج */
    await mutate(d => {
      const p = findPlanProgram(id);
      if (p) Object.assign(p, {name, semester, week, date, targetCount, location, desc});
    });
    EDIT_PLAN_PROGRAM_ID = null; render();
    return;
  }
  ...
  if (action === "markAllStudentsParticipated") {
    const id = btn.dataset.id;
    await mutate(() => { const p = findPlanProgram(id); if (p) p.actualCount = p.targetCount; });
    return;
  }
  if (action === "setPlanProgramCount") {
    const id = btn.dataset.id;
    const input = document.getElementById(`ps-count-${id}`);
    const n = parseInt(input.value, 10);
    if (isNaN(n) || n < 0) return;
    await mutate(() => { const p = findPlanProgram(id); if (p) p.actualCount = n; });
    return;
  }
  ...
  if (action === "playVideoEvidence") {
    VIDEO_LIGHTBOX = { kind: btn.dataset.kind, id: btn.dataset.id, eid: btn.dataset.eid };
    render();
    return;
  }
  if (action === "closeVideoLightbox") { VIDEO_LIGHTBOX = null; render(); return; }
...
document.addEventListener("change", async (e) => {
  if (e.target.classList.contains("evidence-input")) {
    ...
    /* حد الفيديو الخام منخفض عمدًا: قراءة ملف كبير كامل كنص base64 واحد في الذاكرة (ثم حفظه وعرضه)
       هي بالضبط ما كان يُسقط الصفحة ويعيد تحميلها على بعض الأجهزة — حتى بعد إلغاء استخراج اللقطات.
...
(async function init(){
  ...
  if (!DATA.__prunedWeeklyBeforeSY) {
    DATA.weekly = DATA.weekly.filter(w => !(!w.recurring && w.date && w.date < SY_START));
    DATA.__prunedWeeklyBeforeSY = true;
    await persist();
  }
  /* ترميم لمرة واحدة: فقرة الأسبوع الأول من الخطة التشغيلية (٣٠ أغسطس — ٣ سبتمبر) حُذفت عن غير
     قصد (على الأرجح أثناء محاولة حذف فقرة أخرى من قائمة طويلة). نعيدها بمحتواها الأصلي إن كانت
     غائبة، مرة واحدة فقط — إن حذفها المستخدم لاحقًا عن قصد فلن تعود مجددًا */
  if (!DATA.__restoredWeek1Plan) {
    if (!DATA.weeklyPlan.some(p => p.id === "p1")) {
