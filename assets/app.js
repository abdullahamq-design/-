...
const _now = new Date();
const SCHOOL_YEAR_START = "2026-08-30";
let CAL_YEAR = 2026;
let CAL_MONTH = 7; // أغسطس — بداية العام الدراسي
let CAL_SELECTED = SCHOOL_YEAR_START;
let CAL_VIEW = "grid";
let REPORT_MODE = "weekly";
let REPORT_WEEK_ID = null;
let REPORT_YEAR = 2026;
let REPORT_MONTH = 7; // أغسطس — بداية العام الدراسي
  ...
  const detailBody = `
    ...
    ${sel.tasksDue.length ? `
      ...
      <div class="list">
        ${sel.tasksDue.map(t=>{
          const evCount = (t.evidence || []).length;
          const panel = EVIDENCE_OPEN_ID === t.id ? `<div class="item" style="display:block;">${evidencePanel(t)}</div>` : "";
          return `
          <div class="item">
            <button class="check ${t.done?"done":""}" data-action="toggleTask" data-id="${t.id}">${ICONS.check}</button>
            <div class="flex1 title" style="${t.done?'text-decoration:line-through;opacity:.6;':''}">${esc(t.title)}</div>
            ${chip(t.priority, PRIORITY_COLOR[t.priority])}
            <button class="evidence-btn ${evCount?'has':''}" data-action="toggleEvidence" data-id="${t.id}" title="إتمام الإنجاز وإدراج الشواهد">📎${evCount?` ${evCount}`:""}</button>
